...
e           = 0.0015*10^-3;        % roughness of pipes
...
totalTime=300; % total time of system under study in s
...
for i=delT:delT:totalTime
  if Vfill_total>0
    ...
    if Vfill_total>Vfill_critical
        ...
        Vstor_in=sqrt(2*g*headAboveCritical)*(0.25*(Deq^2)*pi)*delT;  %for the ideal case
        %Vstor_in=(0.5*g*headAboveCritical^2)*0.25*(Dfill^2)*pi; %for the ideal case
        vel=(Vstor_in*10^-3)/(pi*0.25*((Deq)^2)*delT)
        %vel=sqrt(2*g*headAboveCritical)*0.001;
        Re_FilltoStor=ReyNo(vel,Deq*10^-3) %input from ideal case
        h_loss=major(Re_FilltoStor,Deq*10^-3,L,vel,e)*1000
        %h_loss=0 %this is to simulate the ideal case

        Vstor_in=sqrt(2*g*(headAboveCritical-h_loss))*(0.25*(Deq^2)*pi)*delT;
    ...
    h_Fill=Vfill_total/(pi*0.25*Dfill^2);

    V_dot_out_burner = sqrt(2*g*h_Fill)*(pi*6.5^2); %what is the outlet dia of the exit? or what is the volume flow rate
